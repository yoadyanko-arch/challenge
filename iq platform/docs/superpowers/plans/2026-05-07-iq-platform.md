# IQ Platform Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a Duolingo-style practical intelligence platform where users scroll smart cards, earn XP, maintain streaks, and compete with friends — backed by AI-generated content.
...
export async function getApprovedCards(supabase: any, pillar?: Pillar, limit = 20) {
  let query = supabase
    .from('cards')
    .select('*')
    .eq('status', 'approved')
...
- [ ] **Step 6: Create `components/feed/FeedHeader.tsx`** (client component for pillar navigation)
...
export default function FeedHeader({ active }: { active: Pillar | 'all' }) {
  const router = useRouter()
  function handleChange(p: Pillar | 'all') {
    router.push(p === 'all' ? '/feed' : `/feed?pillar=${p}`)
  }
  return (
    <div className="sticky top-0 bg-gray-50 pt-4 pb-2 z-10">
      <h1 className="text-xl font-bold px-4 mb-3">🧠 IQ Platform</h1>
      <PillarFilter active={active} onChange={handleChange} />
    </div>
  )
}
```
...
export default async function FeedPage({ searchParams }: { searchParams: Promise<{ pillar?: string }> }) {
  const params = await searchParams
  const pillar = (params.pillar as Pillar) || undefined
  const supabase = await createClient()
  const cards = await getApprovedCards(supabase, pillar, 30)

  return (
    <div className="max-w-lg mx-auto">
      <FeedHeader active={pillar ?? 'all'} />
...
export function buildPrompt(pillar: Pillar, type: CardType, difficulty: Difficulty): string {
  const difficultyCtx = difficulty === 'easy' ? 'simple and accessible' : difficulty === 'medium' ? 'moderately complex' : 'advanced and nuanced'

  return `You are creating content for an IQ improvement platform. Generate a single ${type} card about ${PILLAR_CONTEXT[pillar]}.
...
export async function generateCard(
  ...
  const message = await client.messages.create({
    ...
    max_tokens: 1024,
    messages: [{ role: 'user', content: buildPrompt(pillar, type, difficulty) }],
  })
...
export async function generateBatch(
  ...
  const results = await Promise.all(
    Array.from({ length: count }, () => generateCard(pillar, type, difficulty))
  )
  return results
}
```
...
export async function POST(req: NextRequest) {
  ...
  const { pillar, type, difficulty, count = 5 } = await req.json() as {
    pillar: Pillar; type: CardType; difficulty: Difficulty; count?: number
  }

  const generated = await generateBatch(pillar, type, difficulty, Math.min(count, 20))
...
import { Button } from '@/components/ui/button'
import { Label } from '@/components/ui/label'
import { PILLAR_LABELS, type Pillar, type CardType, type Difficulty } from '@/types'
...
export default function GeneratePage() {
  ...
  async function generate() {
    ...
    const res = await fetch('/api/cards/generate', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ pillar, type, difficulty, count }),
    })
    const data = await res.json()
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <Label>עמודה</Label>
          <select className="w-full border rounded p-2 mt-1" value={pillar} onChange={e => setPillar(e.target.value as Pillar)}>
