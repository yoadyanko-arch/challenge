...
import LearnedSessionCard from '@/components/game/LearnedSessionCard'
...
export default async function ProfilePage() {
  ...
  const [{ data: profile }, { data: progress }, { data: badges }, { data: sessions }] = await Promise.all([
    supabase.from('users').select('*').eq('id', user.id).single(),
    supabase.from('progress').select('*').eq('user_id', user.id),
    supabase.from('user_badges').select('*, badge:badges(*)').eq('user_id', user.id),
    supabase
      .from('learned_sessions')
      .select('id, pillar, topic, score, cards_data, completed_at')
      .eq('user_id', user.id)
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-6 space-y-6 pb-24">
      ...
      {(sessions ?? []).length > 0 && (
        <div className="space-y-3">
          <h2 className="font-semibold text-sm text-muted-foreground uppercase tracking-widest">מה למדתי</h2>
          {(sessions as LearnedSession[]).map(session => (
            <LearnedSessionCard key={session.id} session={session} />
          ))}
        </div>
      )}
